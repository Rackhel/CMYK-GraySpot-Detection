...
### Feature: TrainingWorker 시그널 / TrainingWorker Signals

**Scenario 1: 학습 완료 시 finished 시그널 발생 / finished signal emitted on training completion**
```
...
**Scenario 5: Tab 6 라벨 수정 후 CSV 저장 / CSV saved after Tab 6 label modification**
```
...
**Scenario 6: QApplication 단일 인스턴스 / QApplication single instance**
```
...
## 3. pytest 설정 요구사항 / pytest Configuration Requirements

```ini
# pytest.ini 에 추가 (GUI 테스트용) / Add to pytest.ini (for GUI tests)
[pytest]
qt_api = pyqt6
```

```
# requirements.txt 에 추가 / Add to requirements.txt
PyQt6>=6.4.0
pytest-qt>=4.2.0
```
...
| --- | --- |
| [SSOT_GUI.md](../SSOT/SSOT_GUI.md) | GUI 구조 정의 / GUI structure definition |
| [Contract_gui.md](../Contract/Contract_gui.md) | Worker/탭 API 계약 / Worker/tab API contract |
