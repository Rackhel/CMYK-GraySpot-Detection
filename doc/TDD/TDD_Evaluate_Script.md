...
# [TDD] Evaluate Script — 독립 평가 스크립트

> **목적**: `src/scripts/evaluate.py` CLI 동작을 BDD/TDD로 정의한다.
> **테스트 파일**: `src/tests/unit/test_evaluate_script.py`, `src/tests/integration/test_evaluate_integration.py`
> **상태**: 🔴 Failing — 구현 전
...
### Feature: CLI 평가 스크립트 실행

**Scenario 1: 유효한 체크포인트로 단일 채널 평가**
```
...
**Scenario 3: --channel all 옵션으로 전체 채널 평가**
```
...
**Scenario 4: --output-dir 커스텀 경로 지정**
```
...
def test_evaluate_creates_json_report(mock_checkpoint, tmp_output_dir, minimal_cfg):
    # evaluate.py main() 실행 후 파일 존재 확인
...
| 문서 | 관계 |
| --- | --- |
| [SSOT_Training_Pipeline.md](../SSOT/SSOT_Training_Pipeline.md) | 실행 명령 정의 |
| [Contract_roi_pipeline.md](../Contract/Contract_roi_pipeline.md) | evaluate.py API 계약 |
