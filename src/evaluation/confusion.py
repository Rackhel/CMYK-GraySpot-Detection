...
Generates the 6x6 confusion matrix required by PRD Sections 5.6.2 and 8.2.3.

출력 형식 / Output formats:
    - numpy array (행 정규화 또는 카운트 / row-normalized or raw count)
    - Plotly interactive HTML heatmap
"""
...
def compute_confusion_matrix(
    ...
    """
    6x6 혼동 행렬을 계산한다.
    Computes the 6x6 confusion matrix.
    ...
    Returns:
        cm_raw  : numpy array (num_classes, num_classes) — 카운트 / raw count
        cm_norm : numpy array (num_classes, num_classes) — 정규화 / normalized
                  (normalize=False 이면 cm_raw 와 동일)
    """
