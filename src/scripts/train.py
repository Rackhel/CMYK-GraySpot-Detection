...
from src.utils import setup_logging, get_logger, load_config, validate_config, create_directories, get_nested
...
def step_report(cfg, channels: list[str], device, open_browser: bool, logger) -> bool:
    ...
    from evaluation.metrics import (
        NUM_LEVELS, TARGET_PER_COLOR_ACC, TARGET_MAE,
    ...
    available = [
        r['channel'] for r in summary['results']
    ...
    figs['conf_dist'] = ev_report._build_confidence_dist(all_results, list(all_results.keys()))
    ...
    # CSV / JSON 저장
    ev_report.save_csv(all_results, experiment_name='baseline', channels=list(all_results.keys()))
    ev_report.save_json(
        metrics,
