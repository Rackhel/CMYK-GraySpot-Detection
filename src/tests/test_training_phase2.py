...
def test_config() -> bool:
    ...
    try:
        cfg      = load_config()
        required = ["data", "model", "phase0", "phase2", "storage", "train", "evaluation"]
...
def test_dataset(cfg: dict, channels: list) -> bool:
    ...
    for ch in channels:
        try:
            train_ds = CMYKDataset(cfg, ch, split="train", augment=False, oversample=False)
            val_ds   = CMYKDataset(cfg, ch, split="val",   augment=False, oversample=False)
            test_ds  = CMYKDataset(cfg, ch, split="test",  augment=False, oversample=False)
            ...
            if total == 0:
                ...
                if len(train_ds) > 0:
                    ...
                    if x.shape != (3, size, size):
                        ...
                        pass_(f"[{ch}] 샘플 형태 / Sample shape: {tuple(x.shape)} | Level: {y}")
    ...
    for ch in channels:
        try:
            ...
            if output.shape != (2, expected_dim):
                ...
                pass_(f"[{ch}] 출력 형태 / Output shape: {tuple(output.shape)} (6-class logits)")
...
def test_phase2_training(cfg: dict, channels: list, skip_phase0: bool) -> bool:
    ...
    for ch in channels:
        try:
            train_ds = CMYKDataset(mcfg, ch, split="train", augment=True,  oversample=True)
...
def test_model_saved(cfg: dict, channels: list) -> bool:
    ...
    for ch in channels:
        ...
        if model_path.exists():
            ...
            info_(f"[{ch}] best_{ch}.pt 없음 — 데이터 없어서 학습 미실행 / "
...
def test_performance_targets(cfg: dict, channels: list, skip_phase0: bool) -> bool:
    ...
    for ch in channels:
        ...
        try:
            ...
            print(f"  [{ch}] Test Acc: {test_acc:.4f} (target >= {target_acc}) {acc_icon}")
            print(f"  [{ch}] MAE:      {mae:.4f}      (target <= {target_mae}) {mae_icon}")

            if test_acc < target_acc:
                info_(f"[{ch}] Accuracy 미달 — 데이터 부족 가능 / Below target — possibly insufficient data")
...
def main():
    ...
    results["Phase 2 모델 초기화 / Model Init"]  = test_model_init(cfg, target_channels, args.skip_phase0)
    ...
    results["성능 목표 확인 / Performance"]       = test_performance_targets(cfg, target_channels, args.skip_phase0)
