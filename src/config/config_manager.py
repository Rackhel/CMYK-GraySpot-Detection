...
class ConfigManager(LoggerMixin):
    ...
    def __init__(
        self, config_path: Optional[Path] = None, root_dir: Optional[Path] = None
    ):
        """
        ...
        self.config_path = (
            config_path or self.root_dir / "src" / "config" / "config.yaml"
        )
        self.config = {}
        self.load_config()
        self._resolve_paths()
    ...
    def _find_root_dir(self) -> Path:
        ...
        raise RuntimeError(
            "Could not find project root directory with src/ and Dockerfile"
        )
    ...
    def _setup_device(self) -> None:
        ...
        if device_config == "auto":
            ...
            if gpu_available:
                ...
                raise RuntimeError(
                    "CUDA requested but not available. Use 'auto' or 'cpu'."
                )
            ...
            if not mps_available:
                raise RuntimeError(
                    "MPS requested but not available. Use 'auto', 'cuda', or 'cpu'."
                )
            selected = "mps"
        else:
            raise ValueError(
        ...
        self.config["system"]["device_engine"] = (
            "gpu" if selected in {"cuda", "mps"} else "cpu"
        )
    ...
    def validate(self) -> bool:
        ...
        for *parent_keys, leaf_key in required_keys:
            ...
            if leaf_key not in value:
                self.logger.error(
                    f"Missing required config: {'.'.join(parent_keys + [leaf_key])}"
                )
                return False
    ...
    def __repr__(self) -> str:
        ...
        info.append(
            f"  Channels: {len(self.config['data'].get('channels', []))} "
...
def get_config(
    config_path: Optional[Path] = None, root_dir: Optional[Path] = None
) -> ConfigManager:
    """
