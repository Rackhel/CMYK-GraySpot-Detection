...
def setup_logging(
    ...
    if console:
        ...
        if format_style == "detailed":
            console_formatter = ColoredFormatter(
                console_fmt, datefmt=LoggerConfig.DATETIME_FORMAT
            )
        else:
    ...
    if file and log_dir:
        ...
        file_formatter = logging.Formatter(
            file_fmt, datefmt=LoggerConfig.DATETIME_FORMAT
        )
...
def log_prediction_stats(
    ...
    if len(confs) > 0:
        ...
        logger.info(
            f"  Confidence - Mean: {confs_arr.mean():.4f}, "
...
def log_snapshot(
    ...
    filename = (
        f"config_snapshot_{tag}_{ts}.json" if tag else f"config_snapshot_{ts}.json"
    )
