...
class ChannelCard(QWidget):
    """Y / M / C / K 채널 하나의 레벨 카드 (퍼센테이지·바 없음)."""
    ...
    def __init__(self, channel: str, parent: QWidget | None = None) -> None:
        ...
        self.setStyleSheet("background:#313244; border-radius:10px;")
        ...
        self._ch_lbl.setStyleSheet(f"font-size:20px; font-weight:bold; color:{ch_col};")

        self._level_lbl = QLabel("—")
        self._level_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._level_lbl.setStyleSheet(
            "font-size:42px; font-weight:bold; color:#585b70;"
        )
    ...
    def update_result(self, level: int, confidence: float) -> None:
        ...
        self._level_lbl.setStyleSheet(f"font-size:42px; font-weight:bold; color:{col};")
        self.setStyleSheet(
            f"background:#313244; border:2px solid {col}; border-radius:10px;"
        )
...
class InferenceView(QWidget):
    ...
    def _build_upper_area(self) -> QWidget:
        ...
        v.addWidget(self._img_preview, stretch=1)
        ...
        # Top3 — 내부적으로 데이터는 유지하되 화면에 표시하지 않음
    ...
    def _on_single_done(self, result: dict) -> None:
        ...
        self._status_lbl.setText(f"{t('log_done')} — Lv {pred}  ({conf:.1%})")
        self._set_running(False)
